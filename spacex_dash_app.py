...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Success Count for all')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        rate_df = filtered_df.groupby(['Launch Site', 'class']).size().reset_index(name = 'count')
        fig = px.pie(rate_df, values = 'count', names = 'class', title = f"Total Success Launches for {entered_site}")
        return fig
...
def get_scatter_chart(entered_site, payload):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df2, x = 'Payload Mass (kg)', y = 'class',
                         color = 'Booster Version Category', title = 'Success count on Payload mass for all')
        return fig
    else:
